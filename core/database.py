...
def get_db() -> Session:
    ...
    数据库依赖项，它将在单个请求中使用，然后在请求完成后将其关闭。
    数的返回类型被注解为  Session 是生成的值的类型，而 None 表示生成器没有终止条件。
    """
...
def get_cache(request: Request) -> Redis:
    ...
    if not settings.CACHE_DB_ENABLE:
        raise CustomException("请先配置Redis数据库链接并启用！", desc="请启用 application/settings.py: CACHE_DB_ENABLE")
